...
void ShardMasterServiceImpl::OnNextCommand(Marshallable& m) {
  ...
  if (v->data_[1][0] == 'J'){
    ...
    for (uint32_t datapos = 2; datapos < v->data_.size(); datapos = datapos + 2){
      uint32_t gid = stoi(v->data_[datapos]);
      if (configs_[configNum - 1].group_servers_map_.find(gid) == configs_[configNum - 1].group_servers_map_.end()) {
        int n_group = configs_[configNum - 1].group_servers_map_.size() + 1;
        int minLen = configs_[configNum - 1].shard_group_map_.size() / n_group;

        std::vector<uint32_t> deserialized_v;
        std::stringstream ss(v->data_[datapos + 1]);
        boost::archive::text_iarchive ia(ss);
        ia >> deserialized_v;

        for (int pos = 0; pos < deserialized_v.size(); pos++) config.group_servers_map_[gid].push_back(deserialized_v[pos]);

        map<uint32_t, uint32_t> shard_count;
        for (auto it: config.shard_group_map_) {
          if (it.second == 0 || shard_count[it.second] == minLen) {
  ...
  if (v->data_[1][0] == 'L'){
    ...
    for (int pos = 2; pos < v->data_.size(); pos++) {
      ...
      if (configs_[configNum - 1].group_servers_map_.find(gid) != configs_[configNum - 1].group_servers_map_.end()) {
        int n_group = config.group_servers_map_.size() - 1;

        config.group_servers_map_.erase(gid);
        vector<uint32_t> groups;
        if (config.group_servers_map_.size() == 0) groups.push_back(0);
